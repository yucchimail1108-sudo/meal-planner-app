...
def menu_day_update_view(request, plan_date):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if updated_menu_day.eat_out or updated_menu_day.deli:
                # 献立スロットを全部削除
                for slot in updated_menu_day.slots.all():
                    slot.recipe = None
                    slot.save()
...
# おうち食材を全削除
@login_required
def home_food_delete_all_view(request):
    if request.method != "POST":
        return redirect("recipes:home_food_list")

    target_items = HomeFoodItem.objects.filter(user=request.user)

    deleted_count = target_items.count()
    target_items.delete()

    if deleted_count == 0:
        messages.info(request, "削除するおうち食材がありません")
    else:
        messages.success(request, "おうち食材をすべて削除しました")
